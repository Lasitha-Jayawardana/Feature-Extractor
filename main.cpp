...
int main(int argc, char **argv) {
    ...
    try {
        cout << "Realtime Intrusion Detection System Starting......... " << endl;
        Config config;
    ...
    while (!temination_requested && (has_more_traffic || is_running_live)) {
        ...
        while ((conv = conv_reconstructor.get_next_conversation()) != nullptr) {
            ...
            if (curl) {
                ...
                string url = "http://192.248.81.73/api/v2?id=1&featureVec=";
                const char* data_str = cf->print(config->should_print_extra_features());
                ...
                auto res = readBuffer.find("Anomaly");
                if (std::string::npos != res) {
                    std::cout << "Intrusion Detected\n\n";

                    system("notify-send Caution!' \n Intrusion Detected .\n'");
                    //system("zenity --info --text='Calculation complete'");//msg box
                    cf->print_human();
                }
